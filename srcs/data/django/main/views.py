                ...
                if room:
                    if room.user1 == request.user: #user1とuser2が同一
                        ...
                        response_data = {
                            ...
                            # 生のjavascriptを埋め込みたいとき
                            'rawscripts': 'startGame(' + str(room.id) + ', 2,' +  str(request.user.id) + ', 0, ' + room.paddle_size + ', \'' + is_3d + '\' )',
                        }
                else:
                    rooms = get_available_rooms()
                    tournaments = get_available_tournaments()
                ...
                if room: 
                    if not room.user2:
                        ...
                        response_data = {
                            ...
                            'gameid': str(room.id), 
                            # 生のjavascriptを埋め込みたいとき
                            'rawscripts': 'startGame(' + str(room.id) + ', 1,' +  str(request.user.id) + ', 0, ' + room.paddle_size + ', \'' + is_3d + '\' )',
                        }
                else:
                    rooms = get_available_rooms()
                    tournaments = get_available_tournaments()
                    ...
                    if num_users == 4:
                        ...
                        if room.user1 and room.user2 and room.user3 and room.user4:
                            ...
                            response_data = {
                                ...
                                'gameid': str(room.id), 
                                # 生のjavascriptを埋め込みたいとき
                                'rawscripts': 'startGame(' + str(room.id) + ', ' + str(user_no) + ', ' +  str(user.id) + ', 1, ' + room.paddle_size + ', \'' + is_3d + '\' )',                 
                            }
                        else:
                            response_data = {
                            'page': page,
